...
export const IP_HEADER_CONFIG_PROVIDER = Symbol('IpHeaderConfigProvider');
...
export class AgentAuthGuard implements CanActivate {
  ...
  constructor(
    ...
    @Optional()
    @Inject(IP_HEADER_CONFIG_PROVIDER)
    private readonly ipHeaderConfigProvider?: IpHeaderConfigProvider,
  ) {}
  ...
  private async fetchIpHeaderFromConfig(gameCode: string): Promise<string | undefined> {
    if (this.ipHeaderConfigProvider) {
      try {
        return await this.ipHeaderConfigProvider.getIpHeader(gameCode);
      } catch (error) {
